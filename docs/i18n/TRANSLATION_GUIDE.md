...
When translating a theory content file (`public/data/{locale}/{TheoryName}.json`), the output must have **exactly** the same keys, the same nesting, and the same value types (string vs. array) as the English source file — even where that source file deviates from the documented MTTS v5.0 schema (`src/types/theory.ts`). About 7.5% of the 210 source files have a missing, renamed, or misplaced field, or a string-vs-array mismatch on a field that's usually the other type. **Do not "fix" this.** Translate the values that are there, in the shape they're in. Adding, removing, or renaming a key, or changing a field's JSON type, is treated as a translation error even if the result looks more correct.
