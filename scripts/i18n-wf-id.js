export const meta = {
  name: 'translate-id-theories',
  description: 'Translate 210 theory JSON files into Indonesian (id) via 12 parallel agents',
...
function buildPrompt(files) {
  ...
  return `Translate theory-content JSON files for "Consciousness Atlas" from English into Indonesian (\`id\`), formal academic Bahasa Indonesia. NOTE: Indonesian is NOT identical to Malay (\`ms\`) — do NOT copy Malay phrasing; produce genuine Indonesian. Only touch YOUR files. Do NOT sub-delegate — do the work yourself.

First read fully and follow:
- /Users/dan/Repositories/c-atlas/docs/i18n/TRANSLATION_GUIDE.md
- /Users/dan/Repositories/c-atlas/docs/i18n/glossary.json (has \`id\` renderings for the core terms + MTTS field labels — use them for consistency; do NOT edit. If a recurring domain term isn't in glossary, pick one consistent Indonesian rendering.)
- You MAY consult public/data/fr/{filename} for structure. Translate from the English source at public/data/{filename}.

Convention (all prior locales): translate \`category\`/\`subcategory\`; leave \`related_theories[].name\` verbatim English (cross-references); translate \`related_theories[].relationship\`. Philosopher surnames: Indonesian uses Latin script — keep surnames verbatim (only use an established Indonesian form if one genuinely exists).
...
- \`implications.*.stance\`, \`ontological_status\`, \`primitive_or_emergent_status\`, \`emergence_type\`, \`related_theories[].relationship\` are freeform short prose — translate as short Indonesian prose.
- Empty (""/[]) stays empty. Soul/religious files: neutral register, soul → jiwa/roh consistently; keep transliterated Sanskrit/Hebrew/Arabic terms as source has them.
- Write to /Users/dan/Repositories/c-atlas/public/data/id/{filename} (create the dir if needed).

After all your files, run this and fix any hard failures, re-running until 0:
\`cd /Users/dan/Repositories/c-atlas && node scripts/i18n-check.js id --files ${list.replace(/, /g, ',')}\`
...
const results = await parallel(
  groups.map((files, i) => () =>
    agent(buildPrompt(files), { label: `id-group-${i + 1} (${files.length}f)`, phase: 'Translate' })
  )
)
